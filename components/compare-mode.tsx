...
export function CompareMode() {
  ...
  const currentPlayerColor = getPlayerColorByIndex(
    state.players,
    currentPlayer.id
  );
  ...
  const chartData = Array.from({ length: maxShots }, (_, index) => {
    ...
    const currentCumulative = currentPlayer.scores
      .slice(0, shotNumber)
      .reduce((sum, s) => sum + s, 0);
    const topCumulative = topScorer.scores
      .slice(0, shotNumber)
      .reduce((sum, s) => sum + s, 0);
    ...
    return {
      ...
      currentCumulative:
        currentPlayer.scores[index] !== undefined ? currentCumulative : null,
      topCumulative:
        topScorer.scores[index] !== undefined ? topCumulative : null,
      currentAverage:
        currentPlayer.scores[index] !== undefined
          ? currentCumulative / shotNumber
          : null,
      topAverage:
        topScorer.scores[index] !== undefined
          ? topCumulative / shotNumber
          : null,
    };
  });
      ...
      <CardContent className="space-y-4">
            ...
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 mb-1">
                <div
                  ...
                  style={{ backgroundColor: currentPlayerColor.primary }}
                />
              ...
              <div
                ...
                style={{ color: currentPlayerColor.primary }}
              >
                {currentPlayer.totalScore}
              </div>
              <div className="text-xs text-gray-500">
                Moy: {currentAverage.toFixed(1)} | {currentPlayer.scores.length}{" "}
                tirs
              </div>
            </div>
          </div>
          ...
          >
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 mb-1">
                <div
                  className="w-3 h-3 rounded-full"
                  style={{ backgroundColor: topScorerColor.primary }}
                />
                <div className="text-xs sm:text-sm text-gray-600 truncate">
                  {topScorer.name}
                </div>
              ...
              <div
                className="text-xl sm:text-2xl font-bold"
                style={{ color: topScorerColor.primary }}
              >
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center">
          ...
          <div className="p-2 rounded-lg bg-gray-50 transition-all duration-200 hover:scale-105">
            <div
              className={`text-sm sm:text-lg font-bold ${
                averageDifference >= 0 ? "text-green-600" : "text-red-600"
              }`}
            >
              {averageDifference >= 0 ? "+" : ""}
