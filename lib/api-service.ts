...
class ApiService {
  ...
  async resetPlayerScores(playerId: string): Promise<void> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(
        error.error || "Erreur lors de la réinitialisation des scores"
      );
    }
  }
    ...
    try {
      ...
      for (const localPlayer of state.players) {
        try {
          ...
          console.error(
            `Erreur lors de la migration du joueur ${localPlayer.name}:`,
            error
          );
        }
      }
      ...
      console.error(
        "Erreur lors de la synchronisation depuis localStorage:",
        error
      );
    }
  }
